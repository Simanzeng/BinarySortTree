...
{
	Student stu[_API_MAXSIZE_];             //定义一个大小为_API_MAXSIZE_类型为Student的数组stu
	char a[20]="zeng";
	int Score=90;
	vector<int> vec;                        //定义一个元素类型为int的向量
	for(int i=1;i<=_API_MAXSIZE_;i++)       //插入数据1~50
		vec.push_back(i);
	...
	SortBTree<Student> BST2;                //定义一个二叉排序树对象
	...
	{
		int Rand=rand()%50;                 //随机产生一个要搜索的元素
		Student stu2=stu[Rand];             //stu2为搜索元素

		start1=clock();
		BST2.Search(stu2);                  //从二叉排序树中搜索元素stu2
		finish1=clock();

		start2=clock();
		for(int i=0;i<_API_MAXSIZE_;i++)    //从数组中搜索元素stu2
		{
			if(stu[i]==stu2)
				break;
		}
